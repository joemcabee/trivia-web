...
function ContentView({ eventId }: ContentViewProps) {
  ...
  const handleCreateRound = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedRound && !event) return

    try {
      const newRound = await eventApi.createRound(roundName, eventId)
      setRoundName('')
  ...
  if (loading) {
    return (
      <div className="flex-grow bg-gray-100 dark:bg-gray-900 flex items-center justify-center py-10">
        <div className="text-xl text-gray-900 dark:text-white">Loading content...</div>
      </div>
    )
  }
  ...
  if (!event) {
    return (
      <div className="flex-grow bg-gray-100 dark:bg-gray-900 flex items-center justify-center py-10">
        <div className="text-xl text-gray-900 dark:text-white">Content not found</div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      {currentCategory && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold">Questions - {currentCategory.name}</h2>
            <button
              onClick={() => {
                setEditingQuestionId(null)
                setQuestionText('')
